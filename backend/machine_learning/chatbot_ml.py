...
def doAction(action,shape,color,rel_action=None,rel_shape=None,rel_color=None):
    ...
    x, y = doRelativeAction(rel_action=rel_action, rel_shape=rel_shape, rel_color=rel_color, action=action)
    if rel_action == "NEAR":
        x,y = nearAction(shape=shape,color=color,action=action,rel_action=rel_action,rel_shape=rel_shape,rel_color=rel_color)
