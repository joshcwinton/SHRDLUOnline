...
def drawGrid(draw, scale, rows, cols):
    for i in range(rows):
        draw.line([i * SCALE, 0, i * SCALE, rows * SCALE],
                  fill=(0, 0, 0), width=1)
    for i in range(cols):
        draw.line([0, i * SCALE, cols * SCALE, i * SCALE],
                  fill=(0, 0, 0), width=1)
...
def drawShape(draw, i, j, shape):
    ...
    if shape_name in ("CIRCLE", "SPHERE", "BALL"):
        draw.ellipse([i * SCALE, j * SCALE, (i + 1) * SCALE, (j + 1)
                      * SCALE], fill=color, outline=(0, 0, 0), width=1)
    elif shape_name in ("SQUARE", "BOX", "CUBE"):
        draw.rectangle([i * SCALE, j * SCALE, (i + 1) * SCALE,
                        (j + 1) * SCALE], fill=color, outline=(0, 0, 0), width=1)
        ...
        draw.polygon([top, bottom_left, bottom_right],
