...
def chatbot_test():
    ...
    # res1b = no color, res1c= no shape, res1d = no coordinates res1e = space
    # is already taken
    results.append(
        (GRID[2][2]) == (
            'CUBE',
            'RED',
            0) and res1b == 4 and res1c == 2 and res1d == 10 and res1e == 1)
    ...
    # Delete a shape without coordinates but there is only one copy of that
    # shape
    chatbot(test1a)
    ...
    results.append(
        emptyPosition(
            row=2,
            col=2) and emptyPosition(
            row=1,
            col=1) and emptyPosition(
                row=0,
            col=1))
